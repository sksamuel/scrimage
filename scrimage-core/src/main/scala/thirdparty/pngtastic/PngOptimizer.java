...
public class PngOptimizer {
  ...
  public PngImage optimize(PngImage image, boolean removeGamma, Integer compressionLevel) throws IOException {
    ...
    List<byte[]> originalScanlines = (image.getInterlace() == 1)
        ? pngInterlaceHander.deInterlace((int) image.getWidth(), (int) image.getHeight(), image.getSampleBitCount(),
        inflatedImageData)
  ...
  private PngChunk processHeadChunks(PngImage result,
                                     boolean removeGamma,
                                     Iterator<PngChunk> itChunks) throws IOException {
    PngChunk chunk = null;
