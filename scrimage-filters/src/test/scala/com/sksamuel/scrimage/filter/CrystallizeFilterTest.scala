...
class CrystallizeFilterTest extends FunSuite with Matchers {
  ...
  test("filter output matches expected") {
    ...
    val expected = Image.fromResource("/com/sksamuel/scrimage/filters/bird_small_crystallize.png")
    actual shouldBe expected
  }

  test("support edge thickness") {
    val actual = original.filter(new CrystallizeFilter(16, 0.6, 0xff000000, 0))
    val expected = Image.fromResource("/com/sksamuel/scrimage/filters/bird_small_crystallize_edge_thickeness.png")
    actual shouldBe expected
  }

  test("support edge colour") {
    val actual = original.filter(new CrystallizeFilter(16, 0.6, 0xff00ff00, 0))
    val expected = Image.fromResource("/com/sksamuel/scrimage/filters/bird_small_crystallize_edge_colour.png")
    actual shouldBe expected
  }

  test("support scale") {
    val actual = original.filter(new CrystallizeFilter(12, 0.4, 0xff000000, 0))
    val expected = Image.fromResource("/com/sksamuel/scrimage/filters/bird_small_crystallize_scale.png")
    actual shouldBe expected
  }
