...
class PointillizeFilterTest extends FunSuite with BeforeAndAfter with OneInstancePerTest {
  ...
  test("filter output matches expected") {
    assert(original.filter(PointillizeFilter(Square)) === Image.fromStream(square))
    assert(original.filter(PointillizeFilter(Hexagonal)) === Image.fromStream(hexagonal))
    assert(original.filter(PointillizeFilter(Octangal)) === Image.fromStream(octagonal))
    assert(original.filter(PointillizeFilter(Triangular)) === Image.fromStream(triangular))
  }
}
