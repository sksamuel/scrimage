...
class SmearFilterTest extends FunSuite with BeforeAndAfter with OneInstancePerTest {
  ...
  test("filter output matches expected") {
    assert(original.filter(SmearFilter(Crosses)) === Image.fromStream(expected1))
    assert(original.filter(SmearFilter(Circles)) === Image.fromStream(expected2))
    assert(original.filter(SmearFilter(Squares)) === Image.fromStream(expected3))
  }
}
