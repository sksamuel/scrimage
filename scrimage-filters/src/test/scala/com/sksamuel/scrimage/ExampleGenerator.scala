...
object ExampleGenerator extends App {
  ...
  val filters: List[(String, (String, ImmutableImage) => Filter)] = List(
    ...
    ("ashby", (_, _) => new AshbyFilter()),
    ("brannan", (_, _) => new BrannanFilter()),
    ("brooklyn", (_, _) => new BrooklynFilter()),
    ("charmes", (_, _) => new CharmesFilter()),
    ("clarendon", (_, _) => new ClarendonFilter()),
    ("crema", (_, _) => new CremaFilter()),
    ("dogpatch", (_, _) => new DogpatchFilter()),
    ("earlybird", (_, _) => new EarlybirdFilter()),
    ("gingham", (_, _) => new GinghamFilter()),
    ("ginza", (_, _) => new GinzaFilter()),
    ("hefe", (_, _) => new HefeFilter()),
    ("helena", (_, _) => new HelenaFilter()),
    ("hudson", (_, _) => new HudsonFilter()),
    ("inkwell", (_, _) => new InkwellFilter()),
    ("juno", (_, _) => new JunoFilter()),
    ("kelvin", (_, _) => new KelvinFilter()),
    ("lark", (_, _) => new LarkFilter()),
    ("lofi", (_, _) => new LofiFilter()),
    ("ludwig", (_, _) => new LudwigFilter()),
    ("maven", (_, _) => new MavenFilter()),
    ("mayfair", (_, _) => new MayfairFilter()),
    ("moon", (_, _) => new MoonFilter()),
    ("perpetua", (_, _) => new PerpetuaFilter()),
    ("poprocket", (_, _) => new PoprocketFilter()),
    ("reyes", (_, _) => new ReyesFilter()),
    ("rise", (_, _) => new RiseFilter()),
    ("sierra", (_, _) => new SierraFilter()),
    ("skyline", (_, _) => new SkylineFilter()),
    ("slumber", (_, _) => new SlumberFilter()),
    ("stinson", (_, _) => new StinsonFilter()),
    ("sutro", (_, _) => new SutroFilter()),
    ("toaster", (_, _) => new ToasterFilter()),
    ("valencia", (_, _) => new ValenciaFilter()),
    ("vesper", (_, _) => new VesperFilter()),
    ("walden", (_, _) => new WaldenFilter()),
    ("willow", (_, _) => new WillowFilter()),
    ("xpro_ii", (_, _) => new Xpro2Filter()),
